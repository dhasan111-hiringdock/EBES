...
import { Users, Plus, Trash2, UserCheck, X } from 'lucide-react';
...
export default function RMTeamManagement() {
  ...
  const fetchTeams = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/rm/teams');
      if (response.ok) {
  ...
  const fetchTeamRecruiters = async () => {
    ...
    try {
      const response = await fetch(`/api/rm/team-recruiters/${selectedTeam.id}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchAvailableRecruiters = async () => {
    ...
    try {
      const response = await fetch(`/api/rm/available-recruiters?team_id=${selectedTeam.id}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleAddRecruiter = async () => {
    ...
    try {
      const response = await fetch('/api/rm/team-recruiters', {
        method: 'POST',
  ...
  const handleRemoveRecruiter = async (recruiterId: number) => {
    ...
    try {
      const response = await fetch(`/api/rm/team-recruiters/${selectedTeam.id}/${recruiterId}`, {
        method: 'DELETE'
      });
