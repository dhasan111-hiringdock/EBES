...
app.put("/api/admin/users/:id", adminOnly, async (c) => {
  ...
  try {
    ...
    if ((user as any)?.email === 'danish.hasan@nextlink.ch' && (user as any)?.role === 'admin') {
  ...
  try {
    ...
    for (const user of users) {
      ...
      if (userData.role === "recruiter") {
        ...
        let submissionsQuery = "SELECT * FROM recruiter_submissions WHERE user_id = ?";
        const submissionsParams = [userData.id];

        if (startDate) {
          submissionsQuery += " AND created_at >= ?";
        ...
        for (const sub of submissions) {
          ...
          else if (subData.entry_type === "interview_1st") interviews1st++;
          else if (subData.entry_type === "interview_2nd") interviews2nd++;
          else if (subData.entry_type === "interview_3rd") interviews3rd++;
        ...
        userStats = {
          ...
          activeRoles,
          nonActiveRoles: 0,
          ebesScore: Math.round(ebesScore * 10) / 10,
          performanceLabel,
        };
      } else if (userData.role === "account_manager") {
        // Get roles created by this AM with date filter
        let rolesQuery = "SELECT * FROM am_roles WHERE created_by_user_id = ?";
        ...
        const dealsClosedRoles = roles.filter((r: any) => r.status === "deal_closed").length;
        ...
        const interviewsResult = await db
          .prepare(`
            ...
            WHERE role_id IN (SELECT id FROM am_roles WHERE created_by_user_id = ?)
          `)
          .bind(userData.id)
          .first();
        ...
        const dealsResult = await db
          .prepare(`
            ...
            INNER JOIN recruiter_team_assignments rta ON rs.user_id = rta.recruiter_user_id
            INNER JOIN team_assignments ta ON rta.team_id = ta.team_id
        ...
        const interviewsResult = await db
          .prepare(`
            ...
            INNER JOIN recruiter_team_assignments rta ON rs.user_id = rta.recruiter_user_id
            INNER JOIN team_assignments ta ON rta.team_id = ta.team_id
            WHERE ta.user_id = ? AND rs.entry_type LIKE 'interview%'
          `)
          .bind(userData.id)
          .first();
        const totalInterviews = (interviewsResult as any)?.count || 0;
...
app.get("/api/admin/leaderboards", adminOnly, async (c) => {
  ...
  try {
    ...
    for (const user of users) {
      ...
      if (userData.role === "recruiter") {
        const submissionsResult = await db
          .prepare("SELECT * FROM recruiter_submissions WHERE user_id = ?")
          .bind(userData.id)
          .all();
        const submissions = submissionsResult.results || [];
        ...
        for (const sub of submissions) {
          const subData = sub as any;
          if (subData.entry_type === "submission") totalSubmissions++;
          else if (subData.entry_type.startsWith("interview")) totalInterviews++;
          else if (subData.entry_type === "deal") deals++;
        }

        const table1Points = totalSubmissions * 1.5 + totalInterviews * 3.0 + deals * 7.0;
        ...
        const rolesResult = await db
          .prepare("SELECT * FROM am_roles WHERE created_by_user_id = ?")
          .bind(userData.id)
          .all();
        const roles = rolesResult.results || [];
