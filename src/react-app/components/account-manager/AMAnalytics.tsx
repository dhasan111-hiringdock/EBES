...
export default function AMAnalytics() {
  ...
  const exportToCSV = () => {
    ...
    const csvRows = [];
    csvRows.push("Client,Total Roles,Active,Deals,Lost,On Hold,Cancelled,No Answer,Interviews,Conversion Rate,Health Score,Health Tag");

    analyticsData.clients.forEach((client: any) => {
      csvRows.push([
        client.client_name,
        client.total_roles,
        client.active_roles,
        client.deal_roles,
        client.lost_roles,
        client.on_hold_roles,
        client.cancelled_roles,
        client.no_answer_roles,
        client.total_interviews,
        client.roles_to_deal_conversion,
        client.health_score,
        client.health_tag
      ].join(","));
    });
    ...
    a.href = url;
    a.download = `am-analytics-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  };
