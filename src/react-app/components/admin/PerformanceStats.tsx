...
export default function PerformanceStats() {
  ...
  const handleReportDownload = async (filters: ReportFilters, format: 'csv' | 'excel') => {
    try {
      ...
      if (format === 'csv') {
        generateCSVReport(filteredData);
      } else {
  ...
  const generateCSVReport = (data: UserStats[]) => {
    const csvRows = [];
    ...
    data.forEach((user) => {
      csvRows.push([
        ...
        user.totalRoles || 0,
        user.activeRoles || 0,
        user.nonActiveRoles || 0,
        user.totalSubmissions || 0,
        user.interviews1st || 0,
        user.interviews2nd || 0,
        user.interviews3rd || 0,
        user.totalInterviews || 0,
        user.deals || 0,
        user.dropouts || 0,
        user.dealsClosedRoles || 0,
        user.lostRoles || 0,
        user.onHoldRoles || 0,
        user.noAnswerRoles || 0,
        user.managedTeams || 0,
        user.totalRecruiters || 0,
      ].join(","));
    });
  ...
  const generateExcelReport = (data: UserStats[]) => {
    ...
    data.forEach((user) => {
      csvRows.push([
        ...
        user.interviews3rd || 0,
        user.totalInterviews || 0,
        (user.deals || 0) + (user.dealsClosedRoles || 0),
        user.dropouts || 0,
        user.lostRoles || 0,
        user.onHoldRoles || 0,
        user.noAnswerRoles || 0,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg border-2 border-green-100">
          ...
          <p className="text-gray-600 text-sm font-medium">Overall Success Rate</p>
          <div className="mt-2 text-xs text-gray-500">Submissions to deals conversion</div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg border-2 border-gray-100 p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium text-gray-700">Interview to Deal</span>
                  <span className="text-sm font-bold text-green-600">
                    {totalInterviews > 0 ? ((totalDeals / totalInterviews) * 100).toFixed(1) : 0}%
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-3">
                  <div 
                    className="bg-gradient-to-r from-green-500 to-emerald-500 h-3 rounded-full transition-all"
                    style={{ width: `${Math.min((totalInterviews > 0 ? (totalDeals / totalInterviews) * 100 : 0), 100)}%` }}
                  ></div>
                </div>
              </div>
