...
import { TrendingUp, Target, Award, BarChart3, Users, Filter, X, Clock, HelpCircle, Lightbulb, CheckCircle } from 'lucide-react';
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
          ...
          {analytics?.total_interviews > 0 ? (
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium text-slate-700">3rd Level Interviews</span>
                  <span className="text-sm font-bold text-emerald-600">{analytics.interview_3}</span>
                </div>
                <div className="w-full bg-slate-200 rounded-full h-3">
                  <div 
                    className="bg-gradient-to-r from-emerald-500 to-teal-500 h-3 rounded-full transition-all"
                    style={{ width: `${(analytics.interview_3 / analytics.total_interviews) * 100}%` }}
                  ></div>
                </div>
                <div className="text-xs text-slate-500">
                  {((analytics.interview_3 / analytics.total_interviews) * 100).toFixed(1)}% of total interviews
                </div>
              </div>
              ...
              <div className="mt-6 p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg border-2 border-indigo-200">
                ...
                <div className="text-2xl font-bold text-indigo-600 mt-1">
                  {analytics.interview_1 > 0 
                    ? `${(((analytics.interview_2 + analytics.interview_3) / analytics.interview_1) * 100).toFixed(1)}%`
                    : '0%'}
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
          ...
          <div className="text-4xl font-bold text-emerald-600 mb-2">
            {analytics?.total_deals > 0 ? '12-15' : '0'}
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="w-full bg-slate-200 rounded-full h-2">
              <div 
                ...
                style={{ width: `${Math.min((analytics?.total_submissions > 0 ? (analytics.total_deals / analytics.total_submissions) * 100 : 0), 100)}%` }}
              ></div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
          ...
          <div className="text-4xl font-bold text-purple-600 mb-2">
            {analytics?.total_interviews > 0
              ? `${((analytics.total_deals / analytics.total_interviews) * 100).toFixed(1)}%`
              : '0%'}
          </div>
          <p className="text-sm text-slate-600">interviews converted to deals</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 group cursor-help relative">
            ...
            <p className="text-sm text-indigo-100">
              {analytics?.total_deals > 0 ? 'Deal Closing' : 
               analytics?.total_interviews > 0 ? 'Interview Scheduling' :
               analytics?.total_submissions > 0 ? 'Candidate Sourcing' : 'Getting Started'}
            </p>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 group cursor-help relative">
            ...
            <p className="text-sm text-indigo-100">
              {analytics?.total_interviews > 0 && analytics?.total_deals === 0 ? 'Deal Conversion' :
               analytics?.total_submissions > 0 && analytics?.total_interviews === 0 ? 'Interview Conversion' :
               'Increase Volume'}
            </p>
            <div className="absolute left-0 top-full mt-2 w-72 bg-gray-900 text-white text-sm rounded-lg shadow-xl p-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
              <div className="absolute -top-2 left-4 w-4 h-4 bg-gray-900 transform rotate-45"></div>
      ...
      <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl shadow-lg border-2 border-indigo-200 p-6">
        ...
        <div className="space-y-4">
          {analytics && (() => {
            const actions = [];
            const conversionRate = (analytics?.total_submissions ?? 0) > 0 ? ((analytics?.total_deals ?? 0) / (analytics?.total_submissions ?? 1)) * 100 : 0;
