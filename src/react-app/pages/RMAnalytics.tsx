...
export default function RMAnalytics() {
  ...
  const exportToCSV = () => {
    ...
    let csv = 'RM Analytics Export\n\n';
    
    csv += 'Overview\n';
    csv += `RM EBES Score,${(analytics.overview.rm_ebes_score ?? 0).toFixed(1)}\n`;
    csv += `Total Teams,${analytics.overview.total_teams}\n`;
    csv += `Total Recruiters,${analytics.overview.total_recruiters}\n`;
    csv += `Active Roles,${analytics.overview.total_active_roles}\n`;
    csv += `Deals,${analytics.overview.total_deals}\n`;
    csv += `Dropouts,${analytics.overview.total_dropouts}\n\n`;

    csv += 'Recruiter Performance\n';
    csv += 'Name,Code,Submissions,Interviews,Deals,Dropouts,EBES,Label\n';
    analytics.recruiters.forEach(rec => {
      csv += `${rec.recruiter_name},${rec.recruiter_code},${rec.submissions},${rec.interviews},${rec.deals},${rec.dropouts},${(rec.recruiter_ebes ?? 0).toFixed(1)},${rec.recruiter_ebes_label}\n`;
    });
    ...
    a.href = url;
    a.download = `rm-analytics-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  };
