...
app.get('/api/rm/analytics-comprehensive', rmOnly, async (c) => {
  ...
  try {
    ...
    const recruiterAnalytics = await Promise.all(allRecruiters.map(async (recruiter: any) => {
      ...
      const interview_3 = recSubs.filter((s: any) => s.entry_type === 'interview' && s.interview_level === 3).length;
      ...
      const recEbesData: RecruiterEBESData = {
        ...
        submissions_24h,
        submissions_after_24h,
        interview_1_count: interview_1,
        interview_2_count: interview_2,
        interview_3_count: interview_3,
        deals,
        dropouts,
        accepted_dropouts,
        assigned_roles,
