...
app.post("/api/am/roles", amOnly, async (c) => {
  ...
  try {
    ...
    // Insert additional team assignments if provided
    if (data.team_ids && data.team_ids.length > 0) {
      for (const teamId of data.team_ids) {
        if (teamId !== data.team_id) { // Don't duplicate the primary team
          await db
...
app.put("/api/am/dropout-requests/:id/decide", amOnly, async (c) => {
  ...
  try {
    ...
    if (decision === 'accept') {
      // Accept: Apply new status chosen by AM (or default to 'dropout')
      const finalStatus = new_role_status || 'dropout';
      await db
        .prepare("UPDATE am_roles SET status = ?, updated_at = datetime('now') WHERE id = ?")
        .bind(finalStatus, req.role_id)
        .run();
      
      // Notify recruiter about accepted dropout (-5 points penalty)
      await createNotification(db, {
        userId: req.recruiter_user_id,
        type: 'dropout',
        title: 'Dropout Accepted',
        message: `Your dropout request was accepted by the Account Manager. ${new_role_status ? `Role status updated to ${new_role_status}.` : ''} -5 points applied to your EBES score.`,
        relatedEntityType: 'role',
        relatedEntityId: req.role_id
      });
    } else if (decision === 'ignore') {
      // Ignore: Update to chosen status or keep active, no penalty
      const finalStatus = new_role_status || 'active';
      await db
        .prepare("UPDATE am_roles SET status = ?, updated_at = datetime('now') WHERE id = ?")
        .bind(finalStatus, req.role_id)
        .run();
      
      // Notify recruiter about ignored dropout (no penalty)
      await createNotification(db, {
        userId: req.recruiter_user_id,
        type: 'system',
        title: 'Dropout Ignored',
        message: `Your dropout request was reviewed and determined to not be your fault. ${new_role_status ? `Role status updated to ${new_role_status}.` : 'Role remains active.'} No penalty to your EBES score.`,
        relatedEntityType: 'role',
        relatedEntityId: req.role_id
      });
    }
...
app.get("/api/am/ebes-score", amOnly, async (c) => {
  ...
  try {
    ...
    // Count roles by status (including dropout status)
    const activeRoles = allRoles.filter((r: any) => r.status === "active").length;
    const dealRoles = allRoles.filter((r: any) => r.status === "deal").length;
    const lostRoles = allRoles.filter((r: any) => r.status === "lost").length;
    const noAnswerRoles = allRoles.filter((r: any) => r.status === "no_answer").length;
