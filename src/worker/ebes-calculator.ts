...
export function calculateRecruiterEBES(data: RecruiterEBESData): {
  ...
  table1Points += data.interviews_level_3 * 0;    // 0 points per 3rd interview

  // Deal points
  table1Points += data.deals * 7;  // 7 points per deal

  // Dropout penalty (only accepted dropouts)
  table1Points -= data.accepted_dropouts * 5;  // -5 points per accepted dropout
  
  // Discarded candidates penalty (general discards, not from lost roles)
  table1Points -= data.discarded_candidates * 1.5;  // -1.5 points per discarded candidate
  
  // Lost role candidates penalty (candidates from roles marked as lost)
  table1Points -= data.lost_role_candidates * 1.5;  // -1.5 points per lost role candidate

  // ============ TABLE 2: Role Engagement Points ============
  let table2Points = 0;
  table2Points += data.assigned_roles * 3;        // 3 points per assigned role
  table2Points += data.actively_worked_roles * 1; // 1 point per actively worked role
  ...
  ebesScore = Math.min(100, Math.max(0, ebesScore));
...
export function calculateAccountManagerEBES(data: AccountManagerEBESData): {
  ...
  // EBES = (New Roles × 2) + (Interview 1 × 2) + (Interview 2 × 2) + (Deals × 10) 
  //        - (Lost × 15 per incident) - (No Answer × 10 per incident) 
  //        - (Cancelled × 10 per incident) - (On Hold × 1) - (Dropout × 5)
  //        - (High Active + No Deals penalty)
  ...
  let ebesScore = 
    ...
    (data.deal_roles * 10) -
    (data.lost_roles * 15) -           // -15 points per lost role
    (data.no_answer_roles * 10) -      // -10 points per no answer role
    (data.cancelled_roles * 10) -      // -10 points per cancelled role
    (data.on_hold_roles * 1) -         // -1 point per on hold role
    (data.dropout_roles * 5);          // -5 points per dropout

  // Additional penalty: High active roles with no deals
  if (data.active_roles > 15 && data.deal_roles === 0) {
    ebesScore -= 20;  // -20 points if many active roles but no deals
  }

  // Cap at 100, minimum 0
  const cappedScore = Math.min(100, Math.max(0, ebesScore));
...
export function calculateRecruitmentManagerEBES(data: RecruitmentManagerEBESData): {
  ...
  table1Points += data.total_interviews * 3;  // 3 points per interview
